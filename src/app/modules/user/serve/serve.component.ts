...
export class ServeComponent implements OnInit, OnChanges {
    defaultChecked:any = []
    userId: any
    userRole: any
    userData: any
    ...
    ngOnInit() {
        ...
        this.socket.on('notification_message_serve', (msg: any) => {
    ...
    getOderListing() {
        ...
        this.UserService.getOrderList(formData)
            .subscribe(result => {
                if (result['status'] == 'Success') {
                    this.serve = result['result']['orders'];
                }
            }, (error) => {
            });
    }
    ...
    sendServeStatus(orderId, type = null,sideDishesStatus = null) {
        ...
        if (type == null) {
			...
			for (let i = 0; i < listLi.length; i++) {
				itemIdArr[listLi[i].id] = listLisideDishesStatus[i].id;
                itemIdArr.push(listLi[i].id)
            }
        } else {
				itemIdArr[orderId] = sideDishesStatus;
				}
        var formData = new FormData();
                    ...
                    if (result['status'] == 'Success') {
                        ...
                        if (epson) {
                            ...
                            if (this.printOrder['order']['orderTableId'] != null) {
                                builder.addTextAlign(builder.ALIGN_LEFT).addText('Table #' + this.printOrder['order']['orderTableName'] + ' - Sur place');
